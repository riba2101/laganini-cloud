...
import org.laganini.cloud.storage.filter.FilterSupportFactory;
...
public class ServiceIT {
    ...
    @Autowired
    public ServiceIT(
            FilterSupportFactory filterSupportFactory,
            TestRepository repository
    )
    {
        this.service = new TestService(
                JpaRepositoryHolderFactory.build(repository, query -> query
                        .select(QTestEntity.testEntity)
                        .from(QTestEntity.testEntity)
                ),
                filterSupportFactory.build(new TestSearchCriteriaMapper())
        );
    }
                       ...
                       JpaUpdateableService<Long, TestEntity, TestRepository>
    {
        ...
        private final JpaRepositoryHolder<Long, TestEntity, TestRepository> repositoryHolder;
        private final FilterSupport                                         filterSupport;

        TestService(
                JpaRepositoryHolder<Long, TestEntity, TestRepository> repositoryHolder,
        ...
        @Override
        public JpaRepositoryHolder<Long, TestEntity, TestRepository> getRepositoryHolder() {
