...
import com.laganini.cloud.storage.r2dbc.querydsl.corereactive.dml.InsertClause;
        ...
        implements InsertClause<C>
{
    ...
    public void setBatchToBulk(boolean b) {
        this.batchToBulk = b && configuration
                .getTemplates()
    ...
    public C select(SubQueryExpression<?> sq) {
        this.subQuery = sq;
        for (Map.Entry<ParamExpression<?>, Object> entry : sq
                .getMetadata()
                .getParams()
                .entrySet()) {
    ...
    )
    {
        ...
        if (withKeys) {
            if (entity.getPrimaryKey() != null) {
                String[] target = new String[entity
                        .getPrimaryKey()
                        .getLocalColumns()
                        .size()];
